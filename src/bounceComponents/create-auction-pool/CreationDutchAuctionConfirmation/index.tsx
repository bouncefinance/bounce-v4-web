...
import { DUTCH_AUCTION_CONTRACT_ADDRESSES } from '../../../constants'
import {
...
const CreatePoolButton = ({ times }: { times: string }) => {
  ...
  const createDutchAuctionPool = useCreateDutchAuctionPool()
  ...
  const [approvalState, approveCallback] = useApproveCallback(
    ...
    chainId === auctionInChainId ? DUTCH_AUCTION_CONTRACT_ADDRESSES[auctionInChainId] : undefined,
    true
  )
...
const CreationDutchAuctionConfirmation = () => {
  const values = useValuesState()
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', pb: 48, width: 'fit-content', px: { xs: 16, md: 0 } }}>
        ...
        <Box sx={{ borderRadius: '20px', border: '1px solid #D7D6D9', px: 24, py: 30 }}>
          <Typography variant="h3" sx={{ fontSize: 16, mb: 24 }}>
            {values.poolName} Dutch Auction Pool
          </Typography>
          ...
          <Stack spacing={24}>
            ...
            <Box>
              ...
              <Stack spacing={15}>
                ...
                <ConfirmationInfoItem title="Starting price (price ceiling)">
                  <Typography>
                    1 {values.tokenFrom.symbol} = {values.startPrice} {values.tokenTo.symbol}
                  </Typography>
                </ConfirmationInfoItem>

                <ConfirmationInfoItem title="Reserve price (price floor)">
                  <Typography>
                    1 {values.tokenFrom.symbol} = {values.reservePrice} {values.tokenTo.symbol}
                  </Typography>
                </ConfirmationInfoItem>
...
export default CreationDutchAuctionConfirmation
